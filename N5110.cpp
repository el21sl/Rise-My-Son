...
void N5110::drawReverseSprite(int x0, int y0, int nrows, int ncols, int *sprite){
    for (int i = 0; i < nrows; i++) {
        for (int j = 0 ; j < ncols ; j++) {
            int pixel = *((sprite+i*ncols)+(ncols-1-j));  // pointer of pixel is in reverse column order
            setPixel(x0+j,y0+i, pixel);            
        }
    }
}
